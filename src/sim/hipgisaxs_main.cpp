...
namespace hig {
	...
	HipGISAXS::HipGISAXS(int narg, char **args)
		: freq_(0.0), k0_(0.0), num_layers_(0), num_structures_(0), nqx_(0), nqy_(0), nqz_(0), nqz_extended_(0)
#ifdef USE_MPI
		  ,
    ...
    // TODO first check if the input has been constructed ...
		...
		if (unit == "ev") {
			...
			if (master)
				std::cerr << "error: photon energy is not given in 'ev'" << std::endl;
			return false;
		} // if-else
		...
		if (!QGrid::instance().create(freq_, min_alphai, k0_, mpi_rank)) {
			if (master)
				std::cerr << "error: could not create Q-grid" << std::endl;
			return false;
		} // if
		...
		if (!HiGInput::instance().construct_layer_profile()) { // also can be done at input reading ...
			if (master)
				std::cerr << "error: could not construct layer profile" << std::endl;
			return false;
		} // if
		...
		if (HiGInput::instance().has_substrate_layer()) { //
			...
			substrate_refindex_.delta(0);
			substrate_refindex_.beta(0);
		} // if-else
		...
		if (num_layers_ == 1) {				 // is this really needed? ...
			...
			// initialize
			single_layer_refindex_.delta(0);
			single_layer_refindex_.beta(0);
			single_layer_thickness_ = 0.0;
		} // if-else
		...
		if (!HiGInput::instance().construct_lattice_vectors()) {
			if (master)
				std::cerr << "error: could not construct lattice vectors" << std::endl;
			return false;
		} // if
		...
		if (!HiGInput::instance().compute_domain_size(min_vec, max_vec, z_min_0, z_max_0)) {
			if (master)
				std::cerr << "error: could not compute domain size" << std::endl;
			return false;
		} // if
...
#if defined USE_GPU || defined FF_ANA_GPU || defined FF_NUM_GPU
		if (master)
			std::cout << "-- Waking up GPU(s) ..." << std::flush;
		init_gpu();
		if (master)
			std::cout << " it woke up!" << std::endl;
#elif defined USE_MIC
		if (master)
			std::cout << "-- Waking up MIC(s) ..." << std::flush;
		init_mic();
		if (master)
			std::cout << " done." << std::endl;
#else
		if (master)
			std::cout << "-- Not set up to use any accelerator!" << std::endl;
#endif
...
//      + construct rotation matrices
		...
		if (!illuminated_volume(alphai, HiGInput::instance().scattering_spot_area(),
			...
			if (master)
				std::cerr << "error: something went wrong in illuminated_volume()" << std::endl;
			return false;
		} // if
	...
	bool HipGISAXS::override_qregion(unsigned int ny, unsigned int nz, unsigned int i) {
		...
		if (type == region_qspace) {
			// update Q-grid
			float_t min_alphai = HiGInput::instance().scattering_min_alpha_i() * PI_ / 180;
			if (!QGrid::instance().update(ny, nz, miny, minz, maxy, maxz, freq_, min_alphai, k0_, mpi_rank)) {
				if (master)
					std::cerr << "error: could not update Q-grid" << std::endl;
				return false;
			} // if
	...
	bool HipGISAXS::run_all_gisaxs(int x_min, int x_max, int x_step) {
		...
		if (alphai_max < alphai_min)
			alphai_max = alphai_min;
		if (alphai_min == alphai_max || alphai_step == 0)
			num_alphai = 1;
		else
			num_alphai = (alphai_max - alphai_min) / alphai_step + 1;
		...
		if (phi_step == 0)
			...
			num_phi = (phi_max - phi_min) / phi_step + 1;
		...
		if (tilt_step == 0)
			...
			num_tilt = (tilt_max - tilt_min) / tilt_step + 1;

		if (master) {
			std::cout << "** Num alphai: " << num_alphai << std::endl << "** Num phi: " << num_phi << std::endl
...
// loop over all alphai, phi, and tilt
		...
		if (num_procs > num_alphai) {
			...
			alphai_color = rank;
			alphai_min =
				alphai_min + alphai_step * ((num_alphai / num_procs) * rank + min(rank, num_alphai % num_procs));
		...
		for (int i = 0; i < num_alphai; i++, alpha_i += alphai_step) {
			...
			if (num_procs > num_phi) {
				...
				phi_color = rank;
				phi_min = phi_min + phi_step * ((num_phi / num_procs) * rank + min(rank, num_phi % num_procs));
			...
			for (int j = 0; j < num_phi; j++, phi += phi_step) {
				...
				if (num_procs > num_tilt) {
					...
					tilt_color = rank;
					tilt_min = tilt_min +
						   tilt_step * ((num_tilt / num_procs) * rank + min(rank, num_tilt % num_procs));
				...
				for (int k = 0; k < num_tilt; k++, tilt += tilt_step) {
					...
					if (tmaster) {
						std::cout << "-- Computing GISAXS " << i *num_phi *num_tilt + j *num_tilt + k + 1
							  << " / " << num_alphai *num_phi *num_tilt << " [alphai = " << alpha_i
							  << ", phi = " << phi << ", tilt = " << tilt << "] ..." << std::endl
							  << std::flush;
					} // if
					...
					if (tmaster) {
						...
						alphai_b << alpha_i;
						alphai_s = alphai_b.str();
						phi_b << phi;
						phi_s = phi_b.str();
						tilt_b << tilt;
						tilt_s = tilt_b.str();
						std::string output(HiGInput::instance().param_pathprefix() + "/" +
								   HiGInput::instance().runname() + "/img_ai=" + alphai_s +
								   "_rot=" + phi_s + "_tilt=" + tilt_s + ".tif");
						...
						std::string data_file(HiGInput::instance().param_pathprefix() + "/" +
								      HiGInput::instance().runname() + "/gisaxs_ai=" + alphai_s +
								      "_rot=" + phi_s + "_tilt=" + tilt_s + ".out");
						std::cout << "-- Saving raw data in " << data_file << " ... " << std::flush;
				...
				if (psize > 1) {
					...
					multi_node_.gatherv(phi_comm, averaged_data, proc_sizes[prank], temp_data, proc_sizes,
							    proc_displacements);
					if (pmaster) {
						for (int i = 1; i < psize; ++i) {
			...
			if (asize > 1) {
				...
				multi_node_.gatherv(alphai_comm, averaged_data, proc_sizes[arank], temp_data, proc_sizes,
						    proc_displacements);
				if (amaster) {
					for (int i = 1; i < asize; ++i) {
						add_data_elements(averaged_data, temp_data + proc_displacements[i], averaged_data,
								  nqx_ * nqy_ * nqz_);
					} // for
					delete[] temp_data;
				} // if
	...
	bool HipGISAXS::compute_gisaxs(float_t *&final_data, woo::comm_t comm_key) {
		if (!comm_key.empty())
			sim_comm_ = comm_key; // communicator for this simulation
#ifdef USE_MPI
		bool master = multi_node_.is_master(sim_comm_);
#else
		bool master = true;
		...
		if (num_alphai > 1 || num_phi > 1 || num_tilt > 1) {
			if (master)
				std::cerr << "error: currently you can simulate only for single "
					  << "alpha_i, phi and tilt angles" << std::endl;
			// TODO ...
			return -1.0;
		} // if
		...
		if (master)
			std::cout << "-- Computing GISAXS ... " << std::endl << std::flush;
		/* run a gisaxs simulation */
		if (!run_gisaxs(alpha_i, alphai, phi_rad, tilt_rad, final_data,
#ifdef USE_MPI
				sim_comm_,
		...
		if (master) {
			...
			alphai_b << alpha_i;
			alphai_s = alphai_b.str();
			std::string output(HiGInput::instance().param_pathprefix() + "/" + HiGInput::instance().runname() +
					   "/img_ai=" + alphai_s + ".tif");
			std::cout << "-- Saving image in " << output << " ... " << std::flush;
			img.save(output);
			std::cout << "done." << std::endl;
	...
	bool HipGISAXS::run_gisaxs(float_t alpha_i, float_t alphai, float_t phi, float_t tilt, float_t *&img3d,
				   woo::comm_t comm_key, int corr_doms) {

		SampleRotation rotation_matrix;
		if (!run_init(alphai, phi, tilt, rotation_matrix))
			return false;
		...
		complex_t *fc = NULL;
		if (!compute_propagation_coefficients(alphai, amm, apm, amp, app, rk1, rk2, rk1rk2, tk1tk2, h0, fc)) {
			if (master)
				std::cerr << "error: failed to compute propogation coefficients" << std::endl;
			return false;
		} // if
		...
		for (int s_num = 0; s_num < num_structs && s != HiGInput::instance().structure_end(); ++s, ++s_num) {
			...
			if ((*s).second.grain_is_repetition_dist()) {
				...
				if (scaling_samples.size() > 1) 
                    num_repeats = scaling_samples.size();
				else
					num_repeats = num_grains;
			...
			for (int grain_i = grain_min; grain_i < grain_max; grain_i++) { // or distributions
				...
				if (struct_dist == "gaussian") {
					gauss_weight =
						wght[grain_i] * wght[num_grains + grain_i] * wght[2 * num_grains + grain_i];
				} // if
				...
				mat_mul_3x3(rotation_matrix.r1_, rotation_matrix.r2_, rotation_matrix.r3_, r_norm1, r_norm2,
					    r_norm3, r_tot1, r_tot2, r_tot3);
				...
				mat_mul_3x1(rotation_matrix.r1_, rotation_matrix.r2_, rotation_matrix.r3_, curr_dd_vec, result);
...
/* compute structure factor and form factor */
				...
				for (int i_scale = 0; i_scale < num_repeat_scaling; i_scale++) {
					...
					std::cout << "Calculating scaling-repetition samples " << i_scale + 1 << " of "
				...
				form_factor(ff, shape_name, shape_file, shape_params, curr_transvec, shape_tau, shape_eta, r_tot1,
					    r_tot2, r_tot3
#ifdef USE_MPI
					    ,
					    grain_comm
				...
				if (gmaster) { // grain master
					...
					if (nslices <= 1) {
						...
						if (struct_in_layer &&
							...
							if (single_layer_refindex_.delta() < 0 ||
							    single_layer_refindex_.beta() < 0) {
								...
								for (unsigned int y = 0; y < nqy_; ++y) {
									for (unsigned int x = 0; x < nqx_; ++x) {
										unsigned int curr_index =
											nqx_ * nqy_ * z + nqx_ * y + x;
										unsigned int curr_index_0 = curr_index;
										unsigned int curr_index_1 =
											nqx_ * nqy_ * nqz_ + curr_index;
										unsigned int curr_index_2 =
											2 * nqx_ * nqy_ * nqz_ + curr_index;
										unsigned int curr_index_3 =
											3 * nqx_ * nqy_ * nqz_ + curr_index;
			...
			if (multi_node_.size(struct_comm) > 1) {
				...
				multi_node_.gatherv(struct_comm, grain_ids, gmaster * num_gr * nqx_ * nqy_ * nqz_, id, proc_sizes,
						    proc_displacements);
				delete[] proc_displacements;
				delete[] proc_sizes;
			} else {
					...
					for (unsigned int z = 0; z < nqz_; ++z) {
						for (unsigned int y = 0; y < nqy_; ++y) {
							for (unsigned int x = 0; x < nqx_; ++x) {
								...
								struct_intensity[soffset + curr_index] =
									sum.real() * sum.real() + sum.imag() * sum.imag();
							} // for x
						}	 // for y
					}		  // for z
					break;
		...
		if (multi_node_.size(comm_key) > 1) {
			...
			if (master) {
				if (HiGInput::instance().param_structcorrelation() == structcorr_GE) {
					...
					all_struct_intensity = new (std::nothrow) float_t[num_structures_ * nqx_ * nqy_ * nqz_];
				} // if-else
			}	 // if
			...
			if (HiGInput::instance().param_structcorrelation() == structcorr_GE) {
				multi_node_.gatherv(comm_key, c_struct_intensity, smaster * num_structs * nqx_ * nqy_ * nqz_,
						    all_c_struct_intensity, proc_sizes, proc_displacements);
			} else {
				multi_node_.gatherv(comm_key, struct_intensity, smaster * num_structs * nqx_ * nqy_ * nqz_,
		...
		if (master) {
			...
			if (sigma > 0.0) {
				...
				smear_timer.stop();
				std::cout << "done." << std::endl;
				std::cout << "** Smearing time: " << smear_timer.elapsed_msec() << " ms." << std::endl;
			} // if
		}	 // if master
...
#pragma omp parallel for
		for (unsigned int i = 1; i < size; ++i)
			data[i] = (data[i] - min_val) / d;
		return true;
	} // HipGISAXS::normalize()

	bool HipGISAXS::structure_factor(StructureFactor &sf, std::string expt, vector3_t &center, Lattice *&curr_lattice,
...
#ifndef GPUSF
		return sf.compute_structure_factor(expt, center, curr_lattice, grain_repeats, grain_scaling, r_tot1, r_tot2,
						   r_tot3
#ifdef USE_MPI
...
#else
		return sf.compute_structure_factor_gpu(expt, center, curr_lattice, grain_repeats, grain_scaling, r_tot1, r_tot2,
						       r_tot3
#ifdef USE_MPI
						       , multi_node_, comm_key
#endif
	...
	bool HipGISAXS::form_factor(FormFactor &ff, ShapeName shape_name, std::string shape_file,
...
#ifndef GPUSF
		return ff.compute_form_factor(shape_name, shape_file, shape_params, single_layer_thickness_, curr_transvec,
					      shp_tau, shp_eta, r_tot1, r_tot2, r_tot3
#ifdef USE_MPI
					      , multi_node_, comm_key
#endif
					      );
#else
		return ff.compute_form_factor_gpu(shape_name, shape_file, shape_params, single_layer_thickness_, curr_transvec,
						  shp_tau, shp_eta, r_tot1, r_tot2, r_tot3
#ifdef USE_MPI
						  , multi_node_, comm_key
#endif
	...
	bool HipGISAXS::illuminated_volume(float_t alpha_i, float_t spot_area, int min_layer_order,
		...
		if (HiGInput::instance().experiment() == "saxs") { // SAXS
			...
			if (HiGInput::instance().is_single_layer()) {
				...
				penetration_depth_layer = -1.0 / (2.0 * k0_ * (sqrt(alpha_i * alpha_i - alpha_c * alpha_c -
				...
				std::cerr << "uh-oh: this case (num_layers > 1) has not yet been implemented yet" << std::endl;
				std::cerr << "go get yourself a nice cup of yummmy hot chocolate instead!" << std::endl;
				return false;
			} // if-else
			vol_[0] = vol_[1] = spot_diameter;
			vol_[2] = (penetration_depth_layer.real() < c_max_depth.real()) ? penetration_depth_layer.real()
											: c_max_depth.real();
		} else {
	...
	bool HipGISAXS::compute_propagation_coefficients(float_t alpha_i, complex_t *&amm, complex_t *&apm, complex_t *&amp,
							 complex_t *&app, complex_t *&rk1, complex_t *&rk2, complex_t *&rk1rk2,
							 complex_t *&tk1tk2, complex_t *&h0, complex_t *&fc) {
		...
		if (HiGInput::instance().param_nslices() <= 1) { /* computing without sample slicing */
			complex_t dnl2 = 2.0 * complex_t(single_layer_refindex_.delta(), single_layer_refindex_.beta());
	...
	bool HipGISAXS::compute_fresnel_coefficients_embedded(float_t alpha_i, complex_t *&fc) {
		...
		complex_t a1m_kiz1 = t01_kiz1 / ((float_t)1.0 + r01_kiz1 * r12_kiz1 * exp(complex_t(0, 2) * kiz1 * lt));
		...
		for (unsigned int z = 0; z < nqz_; ++z) {
			...
			if (kfz0 < 0) {
				...
				a1m_nkfz1 = t01_nkfz1 / ((float_t)1.0 + r01_nkfz1 * r12_nkfz1 * temp6);
	...
	bool HipGISAXS::compute_fresnel_coefficients_top_buried(float_t alpha_i, complex_t *&fc) {
		...
		complex_t tk1 =
			((float_t)(2.0 * sin(alpha_i))) / ((float_t)sin(alpha_i) + sqrt((float_t)pow(sin(alpha_i), 2) - dns2_));
	...
	bool HipGISAXS::spatial_distribution(structure_iterator_t s, float_t tz, int dim, int &rand_dim_x, int &rand_dim_y,
					     float_t *&d) {
...
*/ } else {
	...
	bool HipGISAXS::orientation_distribution(structure_iterator_t s, float_t *dd, int ndx, int ndy, float_t *&nn,
						 float_t *&wght) {
	...
	bool HipGISAXS::generate_repetition_range(unsigned int min_val, unsigned int max_val, int num_vals,
	...
	bool HipGISAXS::construct_repetition_distribution(const GrainRepetitions &repetition, int num_grains,
							  std::vector<vector3_t> &all_grains_repeats) {
		...
		case stat_none:
			...
			break;
		case stat_range:
			min_x = repetition.xmin();
			max_x = repetition.xmax();
			generate_repetition_range(min_x, max_x, num_grains, vals_x);
			break;
		case stat_gaussian:
			std::cout << "uh-oh: gaussian distribution for grain repetitions is not yet supported!" << std::endl;
			return false;
			break;
		default:
			std::cerr << "error: invalid grain repetition distribution token encountered" << std::endl;
			return false;
		} // switch
		// for y
		...
		case stat_none:
			...
			break;
		case stat_range:
			min_y = repetition.ymin();
			max_y = repetition.ymax();
		...
		case stat_none:
			...
			break;
		case stat_range:
			min_z = repetition.zmin();
			max_z = repetition.zmax();
	...
	bool HipGISAXS::construct_scaling_distribution(std::vector<StatisticType> dist, vector3_t mean, vector3_t stddev,
						       std::vector<int> nvals, std::vector<vector3_t> &samples,
		...
		for (i = 0; i < 3; i++)
			if (dist[i] != stat_gaussian) {
				std::cerr << "Error: only Gaussian distribution for scaling is implemented." << std::endl;
				return false;
			}
		vector3_t temp;
		vector3_t min, dx;
	...
	bool HipGISAXS::write_qgrid(char *filename) {
		...
		for (unsigned int i = 0; i < nqz_extended_; ++i) {
			qout << QGrid::instance().qz_extended(i).real() << " " << QGrid::instance().qz_extended(i).imag() << " ";
		} // for
		qout << std::endl;
		qout.close();
		return true;
