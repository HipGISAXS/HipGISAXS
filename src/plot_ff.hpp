/***
  ...
  *  File: plot_ff.hpp
  *  Created: Aug 22, 2012
  *  Modified: Wed 22 Aug 2012 06:17:07 PM PDT
  *
	...
	class Plot {
		public:
			Plot(unsigned int nx, unsigned int ny, unsigned int nz):
				...
				raw_data_ = new (std::nothrow) complex_t[nx * ny * 4 * nz];
			...
			bool convert_to_mag() {
				...
				for(int p = 0; p < 4; ++ p) {
					for(unsigned int i = 0; i < nx_ * ny_ * nz_; ++ i) {
						complex_t temp = raw_data_[nx_ * ny_ * nz_ * p + i];
						mag_data_[i] += sqrt(pow(temp.real(), 2.0) + pow(temp.imag(), 2.0));
					} // for
				} // for
				for(unsigned int i = 0; i < nx_ * ny_ * nz_; ++ i) mag_data_[i] = mag_data_[i] / 4;
				return true;
			} // convert_to_mag()
