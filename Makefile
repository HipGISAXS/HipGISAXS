...
NVCC_FLAGS = -Xcompiler -fPIC -Xcompiler -fopenmp -m64 -dc
#NVARCH_FLAGS += -gencode arch=compute_20,code=sm_20
#NVARCH_FLAGS += -gencode arch=compute_20,code=compute_20
#NVARCH_FLAGS += -gencode arch=compute_20,code=sm_21
#NVARCH_FLAGS += -gencode arch=compute_30,code=sm_30
NVARCH_FLAGS += -gencode arch=compute_35,code=sm_35
NVCC_FLAGS += $(NVARCH_FLAGS)
#NVCC_FLAGS += -Xptxas -v -Xcompiler -v -Xlinker -v
NVCC_FLAGS += --ptxas-options="-v"
NVCC_FLAGS += -use_fast_math -ftz=true -prec-div=false
#NVCC_FLAGS += -G -lineinfo #-DFINDBLOCK #-DAXIS_ROT
NVLIB_FLAGS = -Xlinker -lgomp
NVLIB_FLAGS += -Xlinker -Wl,-rpath -Xlinker -Wl,$(CUDA_DIR)/lib64
...
## papi stuff
PAPI_INCL = -I $(PAPI_DIR)/include
PAPI_LIBS = -L $(PAPI_DIR)/lib -lpapi
...
## AMD LibM
#ALIBM_INCL = -I $(HOME)/local/amdlibm-3-0-2/include
#ALIBM_LIBS = -L $(HOME)/local/amdlibm-3-0-2/lib/static -lamdlibm

## Intel MKL
#MKL_INCL = -I /opt/intel/composer_xe_2013.1.117/mkl/include
#MKL_LIBS = -L /opt/intel/composer_xe_2013.1.117/mkl/lib/intel64 -lmkl_core -lmkl_vml_def
...
#MISC_FLAGS += -DPROFILE_PAPI

## for sandy bridge CPU (edison)
#MISC_FLAGS += -DINTEL_SB_AVX
...
#MISC_FLAGS += -DFF_NUM_AUTOTUNE
#MISC_FLAGS += -DFF_NUM_AUTOTUNE_HB
#MISC_FLAGS += -DFF_NUM_GPU_DYNAMICP
endif
endif
...
MISC_FLAGS += -DFF_NUM_MIC_SWAP
MISC_FLAGS += -DFF_NUM_MIC_KB
#MISC_FLAGS += -DMIC_PADDING    ## goes out of memory on target
endif

## CPU related flags
#MISC_FLAGS += -DFF_NUM_CPU_AUTOTUNE_HB
...
#OPT_FLAGS += -march=barcelona -ffast-math -Ofast #-funroll-all-loops
#OPT_FLAGS += -msse4a
#OPT_FLAGS += -U__SSE3__


## stuff for Intel x86_64
#OPT_FLAGS += -mavx -Ofast -falign-functions -align -fma -fp-model fast=2 -parallel -funroll-loops -axAVX -xavx

## for carver CPU
#OPT_FLAGS += -msse4a -ffast-math -Ofast

## for other cpus
ifneq ($(USE_GPU), y)
ifneq ($(USE_MIC), y)
#OPT_FLAGS += -msse  -msse2 -msse3 -mssse3 -msse4.1 -msse4.2 -msse4 -mavx -mavx2 -mfma -msse4a
OPT_FLAGS += -mavx -msse4.1 -msse4.2 -mssse3
endif
endif
...
## all includes
ALL_INCL = $(MPI_INCL) $(CUDA_INCL) $(BOOST_INCL) $(HDF5_INCL) $(TIFF_INCL) $(MISC_INCL) $(PAPI_INCL)

## all libraries
ALL_LIBS = $(BOOST_LIBS) $(MPI_LIBS) $(CUDA_LIBS) $(HDF5_LIBS) $(TIFF_LIBS) $(PAPI_LIBS)
...
ifeq ($(USE_GPU), y)
NV_OBJECTS_MAIN = reduction.o ff_num_gpu.o ff_ana_gpu.o ff_ana_sphere_gpu.o ff_ana_box_gpu.o \
				ff_ana_cylinder_gpu.o ff_ana_hcylinder_gpu.o ff_ana_prism6_gpu.o \
				ff_ana_prism_gpu.o ff_ana_rand_cylinder_gpu.o
OBJECTS_MAIN += nv_hipgisaxs.o $(NV_OBJECTS_MAIN)
else 
ifeq ($(USE_MIC), y)
OBJECTS_MAIN += ff_num_mic.o
else
OBJECTS_MAIN += ff_num_cpu.o
endif
endif

OBJECTS_SIM = $(OBJECTS_MAIN) hipgisaxs_sim.o

OBJECTS_FIT = $(OBJECTS_MAIN) fitting_steepest_descent.o hipgisaxs_fit.o

## the main binary
OBJ_BIN_SIM = $(patsubst %,$(OBJ_DIR)/%,$(OBJECTS_SIM))

NV_OBJ_BIN_SIM = $(patsubst %,$(OBJ_DIR)/%,$(NV_OBJECTS_MAIN))
...
$(OBJ_DIR)/nv_hipgisaxs.o: $(NV_OBJ_BIN_SIM)
	$(NVCC) $^ -o $@ $(NVLINK_FLAGS)
...
clean:
	rm -f $(OBJ_BIN_SIM) $(NV_OBJ_BIN_SIM) $(OBJ_BIN_FIT) $(BIN_DIR)/$(BINARY_SIM) $(BIN_DIR)/$(BINARY_FIT) $(BIN_DIR)/test_conv $(OBJ_DIR)/test_conv.o $(BIN_DIR)/test_read $(OBJ_DIR)/test_read.o $(BIN_DIR)/test_ff $(OBJ_DIR)/test_ff.o $(BIN_DIR)/test_image $(OBJ_DIR)/test_image.o $(OBJ_DIR)/qgrid_test_create.o $(OBJ_DIR)/plot_gisaxs.o $(OBJ_DIR)/plot_ff.o $(OBJ_DIR)/generate_palette.o $(LIB_DIR)/$(LIB_SIM)
